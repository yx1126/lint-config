...
export default async function defineTsConfig(config?: TsConfig): Promise<FlatESLintConfig[]> {
    const { files = [], rules, typeChecked = true } = config || {};
    const tsParser = await interopDefault(import("@typescript-eslint/parser")) as any;

    function makeParser(typeChecked: boolean): FlatESLintConfig {
        return {
            name: `yx1126/typescript${typeChecked ? "/type-checked" : ""}`,
            files: ["**/*.?([cm])ts", "**/*.?([cm])tsx", ...(typeChecked ? [] : files)],
            languageOptions: {
                parser: tsParser,
                parserOptions: {
    ...
    return [
        ...
        ...typeChecked ? [makeParser(false), makeParser(true)] : [makeParser(false)],
        {
        ...
        ...typeChecked
            ? [{
                name: "yx1126/typescript/rules-type-checked",
                files: ["**/*.?([cm])ts", "**/*.?([cm])tsx"],
                rules: defineTsRules(true),
            }]
            : [],
    ];
}